...
pub enum ProviderError {
  /// The daemon socket path does not exist (daemon not running).
...
mod tests {
  ...
  fn display_mentions_path() {
    let err = ProviderError::SocketMissing("/run/tontoo-settings.sock".to_string());
    assert!(err.to_string().contains("/run/tontoo-settings.sock"));
  }
}
